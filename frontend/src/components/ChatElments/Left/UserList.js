...
const UserList = () => {
  ...
  return (
    <div className='h-full '>
      <ul className=' h-[85vh] user-list overflow-hidden '>
        {users.map((user) => (
          <li
            onMouseEnter={() => setIsHover(user._id)}
            onMouseLeave={() => setIsHover('')}
            className={
              actifUser._id === user.user._id
                ? ' flex  cursor-pointer px-3 py-2 h-[4rem] bg-[#f0f2f5]'
                : ' flex  cursor-pointer px-3 py-2 h-[4rem] hover:bg-[#f0f2f5]'
            }
            key={user.user._id}
          ...
          >
            ...
            <div className='basis-[87%] flex justify-between border-b-[1px] border-[#f0f2f5] pb-14'>
              <div className=' flex flex-col'>
                <span className=' text-[1.1rem]'>+{user.user.phone}</span>

                <span>{user?.lastMessage?.message}</span>
              </div>
              <div className=' flex flex-col items-center'>
                <span className=' text-[0.8rem] opacity-80'>19:14</span>
                <span>
                  {isHover == user._id && (
                    <svg
